...
**Play mode (editor):** `DataSOPlayModeEditorBootstrap` on **`ExitingEditMode`** applies default → actual **in memory** (the reset rides into play via Unity's domain-reload backup) so Inspector and runtime agree before `Awake`. It deliberately does **not** `SetDirty` — persisting the re-cloned `_actualData` would churn the asset's managed-reference rids in VCS on every Play, and the on-disk `_actualData` is irrelevant (reset from default at runtime regardless). **`DataSOPlayModeRuntime`** runs **`AfterSceneLoad`** only in the **editor** as a safety net to call **`EnsurePlaySessionInitialized()`** on loaded assets. **Player builds** do not run that scan; **`OnEnable`** (with `Application.isPlaying`) calls **`EnsurePlaySessionInitialized()`** when each asset loads.
